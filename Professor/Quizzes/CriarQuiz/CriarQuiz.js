...
const endpoints = {
    ...
    questionBankItem: (id) => `/questions/bank/${id}`,
...
class QuizApp {
    ...
    bindQuestionBankEvents() {
        document.addEventListener('click', (e) => {
            const questionAction = e.target.closest('.question-actions .btn-icon');
            if (questionAction) {
                const questionItem = questionAction.closest('.question-item');
                const actionType = questionAction.dataset.action;

                if (actionType === 'preview-question') {
            ...
            const materialAction = e.target.closest('.material-actions .btn-icon');
            if (materialAction) {
                const materialItem = materialAction.closest('.material-item');
                const actionType = materialAction.dataset.action || '';

                if (actionType === 'edit-material' || materialAction.title.includes('Editar')) {
                    this.editMaterial(materialItem);
                } else if (actionType === 'remove-material' || materialAction.title.includes('Remover')) {
    ...
    async fetchInitialData() {
        try {
            await Promise.all([this.fetchQuestionBank()]);
        } catch (error) {
            console.error(error);
    ...
    async fetchQuestionBank() {
        const bankContainer = document.querySelector('.question-bank');
        if (!bankContainer) return;

        bankContainer.classList.add('is-loading');
        try {
            const data = await http.get(endpoints.questionBank);
            this.questionBank = Array.isArray(data) ? data : data?.items || [];
            this.populateQuestionBank();
        } catch (error) {
            this.showNotification('Não foi possível carregar o banco de questões.', 'error');
        } finally {
            bankContainer.classList.remove('is-loading');
        }
    }
    ...
    populateQuestionBank() {
        ...
        bankContainer.querySelectorAll('.question-item').forEach(item => item.remove());
        bankContainer.querySelectorAll('.empty-state').forEach(item => item.remove());

        if (!this.questionBank.length) {
            const emptyState = document.createElement('p');
            emptyState.className = 'empty-state';
            emptyState.textContent = 'Nenhuma questão disponível no momento.';
            bankContainer.appendChild(emptyState);
            return;
        }

        const fragment = document.createDocumentFragment();
        this.questionBank.forEach(question => {
            fragment.appendChild(this.createBankQuestionItem(question));
        });

        bankContainer.appendChild(fragment);
    }

    createBankQuestionItem(question) {
        const item = document.createElement('div');
        item.className = 'question-item';
        item.dataset.questionId = question.id;
        item.innerHTML = `
            <div class="question-content">
                <h4>${question.title || question.text}</h4>
                <div class="question-meta">
                    <span class="subject">${question.subjectName || 'Geral'}</span>
                    <span class="type">${this.getQuestionTypeLabel(question.type)}</span>
                    <span class="points">${question.points ?? 0} pontos</span>
                </div>
            </div>
            <div class="question-actions">
                <button class="btn-icon" data-action="preview-question" title="Visualizar questão" aria-label="Visualizar questão">
                    <i class="fas fa-eye"></i>
                </button>
                <button class="btn-icon" data-action="add-question" title="Adicionar questão" aria-label="Adicionar questão ao quiz">
    ...
    mapBankQuestion(bankQuestion) {
        return {
            id: bankQuestion.id,
            text: bankQuestion.text || bankQuestion.title,
            type: bankQuestion.type || 'multipla',
            points: bankQuestion.points || 10,
            options: bankQuestion.options || [],
            correctAnswer: bankQuestion.correctAnswer ?? null,
    ...
    async uploadMaterialFile(file) {
        ...
        const response = await http.upload(endpoints.materialsUpload, formData);
        return response?.url;
    }
    ...
    async saveMaterial() {
        ...
        switch (this.currentMaterialType) {
            ...
            case 'pdf':
                ...
                try {
                    const uploaded = await this.uploadMaterialFile(pdfFile);
                    material.file = {
                        name: pdfFile.name,
                        size: pdfFile.size,
                        type: pdfFile.type
                    };
                    material.url = uploaded?.url || uploaded;
                } catch (error) {
                    this.showNotification(error.message || 'Falha ao enviar PDF.', 'error');
                    return;
                }
                break;
    ...
    async addQuestionFromBank(questionItem) {
        const questionId = questionItem?.dataset?.questionId;
        if (!questionId) {
            this.showNotification('Não foi possível identificar a questão selecionada.', 'error');
            return;
        }

        questionItem?.classList.add('is-loading');
        try {
            const bankQuestion = await http.get(endpoints.questionBankItem(questionId));
            const mappedQuestion = this.mapBankQuestion(bankQuestion);
            ...
            console.error(error);
            this.showNotification(error.message || 'Falha ao adicionar questão.', 'error');
        } finally {
            questionItem?.classList.remove('is-loading');
        }
    }
    ...
    async saveQuiz() {
        ...
        try {
            const method = quizToSave.id ? 'put' : 'post';
            const path = quizToSave.id ? endpoints.quiz(quizToSave.id) : endpoints.quizzes;
            const savedQuiz = await http[method](path, quizToSave);

            this.showNotification('Quiz salvo com sucesso!', 'success');
            localStorage.removeItem('autoSaveQuiz');

            window.dispatchEvent(new CustomEvent('quiz:saved', { detail: savedQuiz }));
        } catch (error) {
            console.error(error);
            this.showNotification(error.message || 'Erro ao salvar quiz. Tente novamente.', 'error');
        } finally {
